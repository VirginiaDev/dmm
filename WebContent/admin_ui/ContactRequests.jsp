  ...
  <%@ include file = "admin-sidebar.jsp" %> 
        ...
        <!-- --------CHECK-BOX START------ -->
			...
			<select name="servicesOptionsNames" class="box" id="servicesOptionsNames">
				    ...
				    <option value="ecommerce">eCommerce Design</option>
				    <option value="webbdev">Web Development</option>
				    <option value="apps">App Design &amp; Development</option>
				    <option value="vr">VR &amp; AR Design</option>
				    <option value="performance">Performance Marketing</option>
				    <option value="print">Print Design</option>
				    <option value="noneselected">None of the above</option>
		     </select>
			</div>
			</div>
			<div class="row col-bg mb-20">
...
alert(search);
	...
	//alert(search);
	$.ajax({
		url: "fetchCustomisedContactRequests.jsp?multiplefiltered=no&search="+search,
		type: 'GET',
		data: null,
...
function submitFilterValues(){
	...
	$.ajax({
		url: "fetchCustomisedContactRequests.jsp?multiplefiltered=yes&fromdate="+fromdate+"&todate="+todate+"&services="+services,
		type: 'GET',
		data: null,
		success: function(data) {
		//$('#ratedemo').val(data);
...
function clearFilterValues(){
	...
	document.getElementById("filters_form").reset();
	 ...
	 $.ajax({
			url: "fetchCustomisedContactRequests.jsp?multiplefiltered=all",
			type: 'GET',
			data: null,
			success: function(data) {
		 //   alert(data);
